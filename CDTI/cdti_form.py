           ...
           if num_target >=1: #存在目标机
                           ...
                           for i in range(1,num_target+1):
                               ...
                               if eval("'target"+str(i)+"_airstatus_txt'") == map_wigdetId[item[0]]:#目标机空地状态
                                   if item[2] == 1:#AIR
                                       ...
                                       eval("self.itp_target" + str(i) + "_airstatus_txt").setText("GROUND")
                       ...
                       if item[1] == 45808:  # 设置飞机应用状态图片
                           ...
                           for i in range(1, num_target+1):
                               if eval("'target"+str(i)+"_applstatus_bitmap'") == map_wigdetId[item[0]]:
                                   ...
                                   eval("self.airb_target"+str(i)+"_applstatus_bitmap").setPixmap(QPixmap("pic/appstatus" + str(airb_app_status) + ".png"))
               ...
               for i in range(len(temp_list_A)):
                   ...
                   for j in range(len(item)):
                       ...
                       item.setBackground(QBrush(QColor(0, 0, 0)))
                       item.setTextAlignment(Qt.AlignCenter)
                       self.ui.tableWidget_surf.setItem(i,j,item)
                   ...
                   if target_pic_surf_type and target_lon_list and target_lat_list:
                       if target_lon_list[i] == 0.0 and target_lat_list[i] == 0.0:#仅tcas surf无法显示目标机
                           ...
                           if target_alt_dif_list and target_air_ground_status_list and target_surf_app_status_list:
                               js_string2 = '''update_target_info(%d,'%s','%s','%s',%d,%f,%f,%d);'''%(i+1,target_id_list[i],str(target_alt_dif_list[i]),target_air_ground_status_list[i],int(target_surf_app_status_list[i]),target_lon, target_lat,ownship_angle)
                               print(js_string2)
                           ...
                           self.browser.page().runJavaScript(js_string1)  # 更新目标机目标
                           if target_alt_dif_list and target_air_ground_status_list and target_surf_app_status_list:
                               js_string2 = '''update_target_info(%d,'%s','%s','%s',%d,%f,%f,%d);'''%(i+1,target_id_list[i],str(target_alt_dif_list[i]),target_air_ground_status_list[i],int(target_surf_app_status_list[i]),target_lon_list[i], target_lat_list[i],ownship_angle)
                               print(js_string2)
